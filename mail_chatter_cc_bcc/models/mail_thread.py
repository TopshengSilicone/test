        ...
        mail = self.env["mail.mail"].sudo().create(
            {
                ...
                "auto_delete": False,
                # Ensure SMTP envelope includes CC/BCC recipients as well.
                "email_to": ", ".join(self._split_emails(", ".join(to_list + cc_list + bcc_list))),
                "email_cc": ", ".join(cc_list) if cc_list else False,
                "email_bcc": ", ".join(bcc_list) if bcc_list else False,
            }
        )
        # Force immediate dispatch so CC/BCC are not blocked by cron queue delays.
        mail.send()
